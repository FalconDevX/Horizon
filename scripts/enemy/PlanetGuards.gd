...
static func spawn_system(
	...
	for planet: Node2D in planets:
		...
		if planet.get("is_star") or planet.get("is_anomaly") or planet.get("is_black_hole"):
			continue
		spawned.append_array(spawn_planet(host, planet, gravity_constant))
	return spawned


static func spawn_planet(host: Node2D, planet: Node2D, gravity_constant: float) -> Array[Enemy]:
	...
	for i in count:
		...
		enemy.title = EnemyMarkers.title_for(enemy_id)
